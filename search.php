...
list($search_info, $num_row) = get_search_info($user_id);
if ($num_row == 1) {
  echo "
...
<div class='navbar' id='navigate-bar' style='background-color: white; height: 100px;'>
    ...
    <div>
        <div style='display: inline-block;'>
            <a class='text-muted' href='search.php?userid=$user_id'>
        ...
        <nav style='display: inline-block;'>
            ...
            <a href='post.php?username={$search_info['username']}'>Post</a>&nbsp;

            <a href='friend.php?userid=$user_id'>Friends</a>&nbsp;

            <a href='profile.php?username={$search_info['username']}'>My Profile</a>&nbsp;
        </nav>

    </div>
</div>
<div class='container-fluid m-1' style='height:630px'>
  <div class='row mt-2'>
    <div class='col-1'></div>
    <div class='col-10 bg-white padding-bottom:100%'>
      <h1 style='text-align: center;margin:20px'>Want do you want to search?</h1>
      <div class='row justify-content-center padding-bottom:100%'>
        
          <form class='form-inline' action='search.php?userid=$user_id' method='POST' style='margin-left:50px'> 
            <div class='col-auto'>
              <input type='text' class='form-control inline-block' name='search_input_post' placeholder='Search Post'></input>
            </div>
            <div class='col-auto'>
              <button class='btn btn-primary inline-block' type='submit'>Search</button>
            </div>
          </form>  
        
          <form class='form-inline' action='search.php?userid=$user_id' method='POST' style='margin-left:50px'> 
            <div class='col-auto'>
              <input type='text' class='form-control inline-block' name='search_input_user' placeholder='Search User' />
            </div>
            <div class='col-auto'>
              <button class='btn btn-primary inline-block' type='submit'>Search</button>
            </div>
      ...
      ";
  if (isset($_POST['search_input_user'])) {
    $search_user = addcslashes($_POST['search_input_user'], '%_');
    $search_input = "%{$search_user}%";
    $stmt = $conn->prepare("select username from user where username like ?");
    $stmt->bind_param("s", $search_input);
    $stmt->execute();
    $result = $stmt->get_result();
    if (mysqli_num_rows($result) == 0) {
      echo "<script> alert('No result!'); window.location='search.php?userid=$user_id'</script>";
    }
    while ($user_search = mysqli_fetch_assoc($result)) {
      echo "
      <div class='container-fluid mt-3 p-3 ' style= 'text-align:center; border-width: 1px'>
          <div class = 'container p-0 mb-1'>
            <a href='friend.php?userid=$user_id&username={$user_search['username']}' style='font-size: 25px'><strong>{$user_search['username']}</strong></a>
          </div>
      </div>
        ";
    }
  } else if (isset($_POST['search_input_post'])) {
    $search_input = $_POST['search_input_post'];
    $stmt = $conn->prepare("CALL SocialNetwork.search(?,?)");
    $stmt->bind_param("si", $search_input, $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    if (mysqli_num_rows($result) == 0) {
      echo "<script>alert('No result!'); window.location='search.php?userid=$user_id'</script>";
    }
    while ($post_search_result = mysqli_fetch_assoc($result)) {
      $conn = mysqli_connect('localhost', 'root', 'root', 'SocialNetwork');
      $friend_post = mysqli_query($conn, "select username from user where id =" . $post_search_result['post_user'] . ";");
      $friend_name = mysqli_fetch_assoc($friend_post);
      $like_num = mysqli_query($conn, "select user_id from likepost where post_id = {$post_search_result['id']}");
      $like_num = mysqli_num_rows($like_num);
      $post_content = nl2br($post_search_result['text_content']);

      echo "
            <div class='container-fluid p-4' style= 'border-bottom-style:solid;border-width: 1px'>
              <div class = 'col-12'>
                <div class = 'container p-0 mb-2'>
                  <strong class = 'd-inline-block mb-2 text-primary'> Post By: <a href = 'friend.php?user_id=$user_id&username={$friend_name['username']}' style='color:#4173D9'>{$friend_name['username']}</a></strong>
                  <div class='btn-group float-right'>
                    <form action='likepost.php' method='POST'>
                      <input type='hidden' name='userid' value=$user_id>
                      <input type='hidden' name='postid' value='{$post_search_result['id']}'>
                ...
                <h3 class = 'mb-3'>{$post_search_result['title']}</h3>
                <div class = 'mb-2 text-muted'> {$post_search_result['post_time']}</div>
                <p class = 'card-text mb-auto' style='font-size: 18px'> $post_content </p>
              
                ";
      if (!is_null($post_search_result['photo_id'])) {
        $conn = mysqli_connect('localhost', 'root', 'root', 'SocialNetwork');
        $result = mysqli_query($conn, "select photo_type, photo_path from photo where id={$post_search_result['photo_id']}");
        $row = mysqli_fetch_assoc($result);
        echo "<img src='{$row["photo_path"]}' width='800' />";
      } else if (!is_null($post_search_result['audio_id'])) {
        $conn = mysqli_connect('localhost', 'root', 'root', 'SocialNetwork');
        $result = mysqli_query($conn, "select audio_type, audio_path from audio where id={$post_search_result['audio_id']}");
        $row = mysqli_fetch_assoc($result);
        echo "<audio width='800'  controls = 'controls' preload='auto'>
                                                    <source src='{$row["audio_path"]}'>
                                                    </audio>";
      } else if (!is_null($post_search_result['video_id'])) {
        $conn = mysqli_connect('localhost', 'root', 'root', 'SocialNetwork');
        $result = mysqli_query($conn, "select video_type, video_path from video where id={$post_search_result['video_id']}");
        $row = mysqli_fetch_assoc($result);
        echo "<video controls ='controls' width='800'  preload='auto'>
